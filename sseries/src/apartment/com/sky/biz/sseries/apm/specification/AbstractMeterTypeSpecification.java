...
public class AbstractMeterTypeSpecification{
	public static Specification<MeterTypeEntity> basicCriteria(Map<String,Object> criteriaMap){
		return new Specification<MeterTypeEntity>() {
			public Predicate toPredicate(Root<MeterTypeEntity> root,CriteriaQuery<?> query,CriteriaBuilder cb){
				MeterTypeDTO dto = null;
				Predicate p1 = cb.equal(root.get("markDelete"), false);
				if(criteriaMap != null)
					dto = (MeterTypeDTO)criteriaMap.get(SpecificationsUtil.DEFAULT_CRITERIA_KEY_MAP);
				if(dto!=null){
