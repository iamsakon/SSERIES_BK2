...
public class AbstractRoomMeterSpecification{
	public static Specification<RoomMeterEntity> basicCriteria(Map<String,Object> criteriaMap){
		return new Specification<RoomMeterEntity>() {
			public Predicate toPredicate(Root<RoomMeterEntity> root,CriteriaQuery<?> query,CriteriaBuilder cb){
				RoomMeterDTO dto = null;
				Predicate p1 = cb.equal(root.get("markDelete"), false);
				if(criteriaMap != null)
					dto = (RoomMeterDTO)criteriaMap.get(SpecificationsUtil.DEFAULT_CRITERIA_KEY_MAP);
				if(dto!=null){
