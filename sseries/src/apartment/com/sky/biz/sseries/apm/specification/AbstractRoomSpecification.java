...
public class AbstractRoomSpecification{
	public static Specification<RoomEntity> basicCriteria(Map<String,Object> criteriaMap){
		return new Specification<RoomEntity>() {
			public Predicate toPredicate(Root<RoomEntity> root,CriteriaQuery<?> query,CriteriaBuilder cb){
				RoomDTO dto = null;
				Predicate p1 = cb.equal(root.get("markDelete"), false);
				if(criteriaMap != null)
					dto = (RoomDTO)criteriaMap.get(SpecificationsUtil.DEFAULT_CRITERIA_KEY_MAP);
				if(dto!=null){
