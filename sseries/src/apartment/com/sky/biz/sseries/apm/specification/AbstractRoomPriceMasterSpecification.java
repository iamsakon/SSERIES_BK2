...
public class AbstractRoomPriceMasterSpecification{
	public static Specification<RoomPriceMasterEntity> basicCriteria(Map<String,Object> criteriaMap){
		return new Specification<RoomPriceMasterEntity>() {
			public Predicate toPredicate(Root<RoomPriceMasterEntity> root,CriteriaQuery<?> query,CriteriaBuilder cb){
				RoomPriceMasterDTO dto = null;
				Predicate p1 = cb.equal(root.get("markDelete"), false);
				if(criteriaMap != null)
					dto = (RoomPriceMasterDTO)criteriaMap.get(SpecificationsUtil.DEFAULT_CRITERIA_KEY_MAP);
				if(dto!=null){
